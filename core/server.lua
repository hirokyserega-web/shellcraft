...
function server.run()
    ...
    -- 4. Связь событий -> UI + лог
    local uiInstance = nil
    local function onEvent(etype, payload)
        local msg = etype
        if payload then
            if payload.recipe then msg = msg .. " " .. lang.localize(payload.recipe)
            elseif payload.id then msg = msg .. " " .. lang.localize(payload.id)
        ...
        if uiInstance then uiInstance:addLog(msg) end
    end
